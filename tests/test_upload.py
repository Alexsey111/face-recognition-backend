...
class TestUploadEndpoints:
    ...
    def test_create_upload_session(self, client):
        ...
        with (
            patch(
                "app.routes.upload.SessionService.create_session",
                new_callable=AsyncMock,
            ) as mock_create,
            patch(
                "app.db.crud.AuditLogCRUD.log_action", new_callable=AsyncMock
            ) as mock_log,
        ):
            ...
            response = client.post(
                "/api/v1/upload/", headers={"Authorization": "Bearer test_token"}
            )
    ...
    def test_get_upload_status(self, client):
        ...
        with patch(
            "app.routes.upload.SessionService.get_session", new_callable=AsyncMock
        ) as mock_get:
    ...
    def test_delete_upload_session(self, client):
        ...
        with (
            ...
            patch(
                "app.routes.upload.SessionService.delete_session",
                new_callable=AsyncMock,
            ) as mock_delete,
...
class TestUploadWorkflows:
    ...
    def test_complete_upload_workflow(self, client):
        ...
        with (
            ...
            patch(
                "app.routes.upload.SessionService.validate_session",
                new_callable=AsyncMock,
            ) as mock_validate,
            patch(
                "app.routes.upload.SessionService.attach_file_to_session",
                new_callable=AsyncMock,
            ) as mock_attach,
