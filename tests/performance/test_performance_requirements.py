...
class TestPerformanceRequirements:
    ...
    def test_images(self):
        ...
        for img_path in list(test_dir.glob("*.jpg"))[:50]:
            images.append(
                ImageFileHandler.load_image_from_bytes(
                    img_path.read_bytes(), img_path.name
                )
            )
    ...
    async def test_single_verification_time(
        self, verification_service, anti_spoofing_service, test_images
    ):
        """
        ...
        assert (
            mean_time < 1.0
        ), f"Mean time {mean_time:.3f}s exceeds 1 second requirement"
    ...
    async def test_throughput(self, verification_service, test_images):
        ...
        tasks = [verification_service.extract_embedding(img) for img in images_batch]
    ...
    @pytest.mark.asyncio
    async def test_different_image_sizes(self, verification_service, image_size):
        """
        ...
        assert elapsed < 1.0, f"Processing {width}x{height} took {elapsed:.3f}s (> 1s)"
