...
class TestCleanupTasks:
    ...
    async def test_cleanup_expired_upload_sessions(self):
        ...
        with patch('app.tasks.cleanup.CacheService') as mock_cache, \
    ...
    async def test_cleanup_old_files_from_storage(self):
        ...
        with patch('app.tasks.cleanup.StorageService') as mock_storage, \
    ...
    async def test_cleanup_old_verification_sessions(self):
        ...
        with patch('app.tasks.cleanup.get_async_db_manager') as mock_db_manager, \
            ...
            mock_db_service_instance.verification_crud.cleanup_old_sessions = AsyncMock(return_value=10)
    ...
    async def test_cleanup_old_logs(self):
        ...
        with patch('app.tasks.cleanup.get_async_db_manager') as mock_db_manager, \
            ...
            mock_db_service_instance = Mock()
            mock_db_service.return_value = mock_db_service_instance
            mock_db_service_instance.audit_crud.cleanup_old_logs = AsyncMock(return_value=100)
    ...
    async def test_run_full_cleanup(self):
        ...
        with patch.object(CleanupTasks, 'cleanup_expired_upload_sessions', new_callable=AsyncMock) as mock_sessions, \
             patch.object(CleanupTasks, 'cleanup_old_files_from_storage', new_callable=AsyncMock) as mock_files, \
             patch.object(CleanupTasks, 'cleanup_old_verification_sessions', new_callable=AsyncMock) as mock_verif, \
             patch.object(CleanupTasks, 'cleanup_old_logs', new_callable=AsyncMock) as mock_logs:
