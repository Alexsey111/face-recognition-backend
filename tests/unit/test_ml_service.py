...
def create_mock_ml_service():
    ...
    mock_service.generate_embedding = AsyncMock(return_value={
        'success': True,
        'face_detected': True,
        'embedding': [0.1] * 512,
        'quality_score': 0.85
    ...
    mock_service.verify_face = AsyncMock(return_value={
        'success': True,
        'verified': True,
        'similarity_score': 0.92
    })
    
    mock_service.check_liveness = AsyncMock(return_value={
        'success': True,
        'liveness_detected': True,
        'confidence': 0.88
    })
    
    mock_service.batch_generate_embeddings = AsyncMock(return_value=[
        {'success': True, 'face_detected': True, 'embedding': [0.1] * 512}
        for _ in range(5)
    ])
