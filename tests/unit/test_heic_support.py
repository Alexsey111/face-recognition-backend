...
class TestHEICFileUtils:
    ...
    def test_validate_image_file_too_large(self):
        ...
        from app.utils.file_utils import validate_image_file, MAX_FILE_SIZE
...
def test_images_dir(tmp_path):
    ...
    for i in range(3):
        ...
        (images_dir / f"person{i+1}_img1.jpg").write_bytes(
            _image_to_bytes(img, 'JPEG')
        )
        (images_dir / f"person{i+1}_img2.jpg").write_bytes(
            _image_to_bytes(img, 'JPEG')
        )
