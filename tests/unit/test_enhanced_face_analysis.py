...
class TestFaceAlignment:
    ...
    def test_align_face_output_size(self, sample_face_image, sample_landmarks):
        ...
        output_size = (112, 112)
        aligned_face, metadata = align_face(
            sample_face_image, sample_landmarks, output_size=output_size
        )
...
class TestDepthEstimation:
    ...
    def real_face_like(self):
        ...
        eyes = np.exp(-((xx - 0.3) ** 2 + (yy - 0.3) ** 2) / 0.05) + np.exp(
            -((xx - 0.7) ** 2 + (yy - 0.3) ** 2) / 0.05
        )
...
class TestIntegration:
    ...
    def test_full_pipeline(self):
        ...
        x = np.linspace(0, 255, w)
        face = np.clip(
            face.astype(np.int16) + x.astype(np.int16)[np.newaxis, :, np.newaxis],
            0,
            255,
        ).astype(np.uint8)
