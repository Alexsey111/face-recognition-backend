...
class TestReferenceRoutes:
    ...
    def sample_image_data(self):
        ...
        return (
            f"data:image/jpeg;base64,{base64.b64encode(img_bytes.getvalue()).decode()}"
        )
    ...
    def test_get_references_with_pagination(self, app, sample_reference_data):
        ...
        with TestClient(app, raise_server_exceptions=False) as client:
            response = client.get(
                "/api/v1/reference?page=2&per_page=10&user_id=user123"
            )
    ...
    def test_get_reference_success(self, client, sample_reference_data):
        ...
        with (
            patch(
                "app.db.crud.ReferenceCRUD.get_reference_by_id", new_callable=AsyncMock
            ) as mock_get_ref,
    ...
    def test_create_reference_success(
        self, client, sample_reference_data, sample_image_data
    ):
        reference_id = str(uuid.uuid4())
        ...
        mock_validation_result.image_data = (
            sample_image_data.encode()
            if isinstance(sample_image_data, str)
            else sample_image_data
        )
        ...
        mock_validation_instance.validate_image = AsyncMock(
            return_value=mock_validation_result
        )
        mock_validation_instance.aclose = AsyncMock()

        mock_storage_instance = Mock()
        mock_storage_instance.upload_image = AsyncMock(
            return_value={
                "file_url": "http://minio/test-image.jpg",
                "file_size": 102400,
        ...
        mock_ml_instance.generate_embedding = AsyncMock(
            return_value={
                "success": True,
        ...
        mock_encryption_instance.encrypt_embedding = AsyncMock(
            return_value=b"encrypted_embedding"
        )
        ...
        with (
            ...
            patch(
                ...
                return_value=mock_encryption_instance,
            ),
            patch(
                "app.db.crud.ReferenceCRUD.create_reference", new_callable=AsyncMock
            ) as mock_create_ref,
            patch("app.db.database.get_async_db_manager", return_value=mock_db_manager),
        ):
    ...
    def test_update_reference_success(self, client, sample_reference_data):
        ...
        with (
            ...
            patch(
                "app.db.crud.ReferenceCRUD.update_reference", new_callable=AsyncMock
            ) as mock_update_ref,
            patch("app.db.database.get_async_db_manager", return_value=mock_db_manager),
        ):
            ...
            response = client.put(
                f"/api/v1/reference/{reference_id}", json=request_data
            )
    ...
    def test_delete_reference_success(self, client, sample_reference_data):
        ...
        with (
            ...
            patch(
                "app.db.crud.ReferenceCRUD.delete_reference", new_callable=AsyncMock
            ) as mock_delete_ref,
            patch(
                "app.services.storage_service.StorageService",
                return_value=mock_storage_instance,
    ...
    def test_compare_with_references_success(
        self, client, sample_reference_data, sample_image_data
    ):
        ...
        mock_encryption_instance = Mock()
        mock_encryption_instance.decrypt_embedding = AsyncMock(
            return_value=[0.1, 0.2, 0.3]
        )

        mock_ml_instance = Mock()
        mock_ml_instance.compare_faces = AsyncMock(
            return_value={
                "success": True,
        ...
        with (
            ...
            patch(
                "app.db.crud.ReferenceCRUD.get_all_references", new_callable=AsyncMock
            ) as mock_get_refs,
            patch("app.db.database.get_async_db_manager", return_value=mock_db_manager),
        ):
    ...
    def test_compare_with_references_no_user_or_ids(self, client, sample_image_data):
        mock_session = create_mock_db_session()
        mock_session.execute = AsyncMock(
            return_value=Mock(scalars=Mock(all=Mock(return_value=[])))
        )
        mock_db_manager = create_mock_db_manager(mock_session)
