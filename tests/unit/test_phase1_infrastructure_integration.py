...
class TestInfrastructureIntegration:
    ...
    def test_database_url_config(self, config_content):
        ...
        assert "DATABASE_URL" in config_content, "Должна быть конфигурация DATABASE_URL"
        assert (
            "postgresql" in config_content
    ...
    def test_redis_url_config(self, config_content):
        ...
        assert "REDIS_URL" in config_content, "Должна быть REDIS_URL конфигурация"
        assert "redis://" in config_content, "REDIS_URL должен начинаться с redis://"

    def test_redis_pool_size_config(self, config_content):
        """Тест: Redis pool size настроен."""
        assert (
            "REDIS_CONNECTION_POOL_SIZE" in config_content
    ...
    def test_cache_service_methods(self, project_root):
        ...
        assert (
            "async def get" in content or "def get" in content
        ), "CacheService должен иметь метод get"
        assert (
            "async def set" in content or "def set" in content
        ), "CacheService должен иметь метод set"
        assert (
            "async def delete" in content or "def delete" in content
        ), "CacheService должен иметь метод delete"
        # Проверяем наличие других ключевых методов вместо health_check
        assert (
            "async def check_rate_limit" in content or "def check_rate_limit" in content
    ...
    def test_encryption_key_config(self, config_content):
        ...
        assert (
            "ENCRYPTION_KEY" in config_content
    ...
    def test_encryption_algorithm_config(self, config_content):
        ...
        assert (
            "ENCRYPTION_ALGORITHM" in config_content
        ), "Должна быть ENCRYPTION_ALGORITHM конфигурация"
        assert (
            "aes-256-gcm" in config_content.lower()
    ...
    def test_cors_origins_config(self, config_content):
        ...
        assert "CORS_ORIGINS" in config_content, "Должна быть CORS_ORIGINS конфигурация"
    ...
    def test_thresholds_exist(self, config_content):
        ...
        has_thresholds = (
            "THRESHOLD" in config_content or "threshold" in config_content.lower()
        )
        assert has_thresholds, "Должны быть пороговые значения для верификации"
    ...
    def test_alembic_env_py_imports_base(self, project_root):
        ...
        assert (
            "from app.db.models import Base" in content
    ...
    def test_docker_compose_has_redis_service(self, project_root, compose_file):
        ...
        services = config.get("services", {})
        assert "redis" in services, f"{compose_file} должен содержать сервис redis"
    ...
    def test_docker_compose_has_minio_service(self, project_root, compose_file):
        ...
        services = config.get("services", {})
        assert "minio" in services, f"{compose_file} должен содержать сервис minio"
    ...
    def test_docker_compose_has_volumes(self, project_root, compose_file):
        ...
        assert "volumes" in config, f"{compose_file} должен определять volumes"
    ...
    def test_docker_compose_has_networks(self, project_root, compose_file):
        ...
        assert "networks" in config, f"{compose_file} должен определять networks"
...
class TestDatabaseModelsIntegration:
    ...
    def test_base_model_exists(self, project_root):
        ...
        assert (
            "from .database import Base" in content
            or "from database import Base" in content
    ...
    def test_models_use_base(self, project_root):
        ...
        assert (
            "Base)" in content or "(Base)" in content
