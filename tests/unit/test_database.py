...
class TestBaseCRUD:
    ...
    async def test_update_not_found(self):
        ...
        assert result is None
        # Проверяем, что метод update был вызван
        self.mock_crud.update.assert_called_once_with(
            mock_db, "nonexistent_id", obj_data
        )
...
class TestUserOperations:
    ...
    async def test_get_by_email(self):
        ...
        self.mock_user_crud.get_by_email.assert_called_once_with(
    ...
    async def test_get_by_api_key(self):
        ...
        # Выполняем операцию
        result = await self.mock_user_crud.get_by_key(
            mock_db, "sk_test_1234567890abcdef"
        )

        assert result == mock_user
        # Проверяем, что метод get_by_key был вызван
        self.mock_user_crud.get_by_key.assert_called_once_with(
            mock_db, "sk_test_1234567890abcdef"
        )
    ...
    async def test_update_last_login(self):
        ...
        self.mock_user_crud.update_last_login.assert_called_once_with(
            mock_db, "test_user_id"
        )
    ...
    async def test_increment_stats(self):
        ...
        # Выполняем операцию
        result = await self.mock_user_crud.increment_stats(
            mock_db, "test_user_id", "requests", 5
        )

        assert result == mock_user
        # Проверяем, что метод increment_stats был вызван
        self.mock_user_crud.increment_stats.assert_called_once_with(
            mock_db, "test_user_id", "requests", 5
        )
...
class TestReferenceOperations:
    ...
    async def test_get_by_user(self):
        ...
        self.mock_reference_crud.get_by_user.assert_called_once_with(
            mock_db, "test_user_id"
        )
    ...
    async def test_get_by_label(self):
        ...
        mock_db.execute.return_value = (
            Mock(
                scalars=Mock(return_value=Mock(first=Mock(return_value=mock_reference)))
            ),
        )

        # Имитируем вызов get_by_label
        self.mock_reference_crud.get_by_label = AsyncMock(return_value=mock_reference)
        result = await self.mock_reference_crud.get_by_label(
            mock_db, "test_user_id", "test_label"
        )

        assert result == mock_reference
        # Проверяем что метод crud был вызван с правильными аргументами
        self.mock_reference_crud.get_by_label.assert_called_once_with(
            mock_db, "test_user_id", "test_label"
        )
    ...
    async def test_increment_usage(self):
        ...
        self.mock_reference_crud.increment_usage = AsyncMock(
            return_value=mock_reference
        )
        result = await self.mock_reference_crud.increment_usage(
            mock_db, "test_reference_id"
        )

        assert result == mock_reference
        # Проверяем что метод crud был вызван
        self.mock_reference_crud.increment_usage.assert_called_once_with(
            mock_db, "test_reference_id"
        )
...
class TestVerificationSessionOperations:
    ...
    async def test_get_active_sessions(self):
        ...
        mock_db.execute.return_value = Mock(
            scalars=Mock(return_value=Mock(all=Mock(return_value=mock_sessions)))
        )

        # Имитируем вызов get_active_sessions
        self.mock_session_crud.get_active_sessions = AsyncMock(
