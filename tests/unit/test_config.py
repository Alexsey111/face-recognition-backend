...
class TestSettings:
    ...
    def test_settings_from_env(self):
        ...
        settings_obj = Settings()
        assert (
            settings_obj.DATABASE_URL == "postgresql://test:test@localhost:5432/test_db"
        )
    ...
    def test_validate_debug(self):
        ...
        with patch.dict(os.environ, {"ENVIRONMENT": "production", "DEBUG": "true"}):
            with pytest.raises(
                ValueError,
                match="DEBUG mode cannot be enabled in production environment",
            ):
                Settings()
    ...
    def test_debug_validation_production_fails(self, monkeypatch):
        ...
        with pytest.raises(
            ValueError, match="DEBUG mode cannot be enabled in production environment"
        ):
            Settings()
    ...
    def test_s3_configuration(self):
        ...
        assert (
            settings_obj.S3_BUCKET_NAME == "face-recognition"
