...
class TestDockerProductionInfrastructure:
    ...
    def test_docker_compose_prod_has_api_service(self, docker_compose_prod):
        ...
        assert "api" in services, "docker-compose.prod.yml должен содержать сервис 'api'"
    ...
    def test_docker_compose_prod_api_container_name(self, docker_compose_prod):
        ...
        assert api_service["container_name"] == "face_recognition_api", \
    ...
    def test_docker_compose_prod_api_environment(self, docker_compose_prod):
        ...
        assert env["ENVIRONMENT"] == "production", "ENVIRONMENT должен быть 'production'"
        ...
        assert "postgres" in env["DATABASE_URL"], "DATABASE_URL должен указывать на postgres"
        ...
        assert "minio" in env["S3_ENDPOINT_URL"].lower(), \
    ...
    def test_docker_compose_prod_postgres_healthcheck(self, docker_compose_prod):
        ...
        assert "test" in postgres_service["healthcheck"], "Должен быть test в healthcheck"
    ...
    def test_docker_compose_prod_minio_healthcheck(self, docker_compose_prod):
        ...
        assert "curl" in test_cmd or "health" in test_cmd.lower(), \
    ...
    def test_docker_compose_prod_pgadmin_profile(self, docker_compose_prod):
        ...
        if "pgadmin" in services:
            ...
            assert "profiles" in pgadmin_service, \
                "pgAdmin должен использовать profiles для опционального запуска"
            assert "admin" in pgadmin_service["profiles"], \
    ...
    def test_docker_compose_prod_networks(self, docker_compose_prod):
        ...
        assert "face_recognition_network" in networks, \
    ...
    def test_dockerfile_production_multi_stage(self, dockerfile_prod):
        ...
        assert "FROM python:" in dockerfile_prod, "Должен использовать базовый образ Python"
    ...
    def test_dockerfile_production_non_root_user(self, dockerfile_prod):
        ...
        assert "useradd" in dockerfile_prod or "groupadd" in dockerfile_prod, \
            "Должен создавать пользователя/группу"
        assert "USER" in dockerfile_prod, "Должен переключаться на non-root пользователя"

    def test_dockerfile_production_apt_cleanup(self, dockerfile_prod):
        """Тест: Dockerfile очищает apt cache."""
        assert "apt-get clean" in dockerfile_prod or "rm -rf /var/lib/apt/lists" in dockerfile_prod, \
    ...
    def test_dockerfile_production_requirements_cache(self, dockerfile_prod):
        ...
        for line in lines:
            if "COPY requirements" in line:
                req_copied = True
            if line.strip().startswith("COPY .") or line.strip().startswith("COPY --chown"):
                code_copied = True
    ...
    def test_dockerfile_production_no_cache_dir(self, dockerfile_prod):
        ...
        assert "--no-cache-dir" in dockerfile_prod, \
    ...
    def test_dockerfile_production_uvicorn(self, dockerfile_prod):
        ...
        assert "app.main:app" in dockerfile_prod or "app:app" in dockerfile_prod, \
    ...
    def test_dockerfile_production_healthcheck(self, dockerfile_prod):
        ...
        assert "8000" in dockerfile_prod or "health" in dockerfile_prod.lower(), \
...
class TestDockerDevelopmentInfrastructure:
        ...
        if "depends_on" in api_service:
            ...
            for service, config in depends.items():
                if isinstance(config, dict):
                    ...
                    assert config["condition"] == "service_healthy", \
    ...
    def test_docker_compose_dev_hot_reload_config(self, docker_compose_dev):
        ...
        assert "ENVIRONMENT" in env_str or "development" in env_str, \
    ...
    def test_docker_compose_dev_additional_services(self, docker_compose_dev):
        ...
        services = docker_compose_dev.get("services", {})
        assert "redis-commander" in services, \
    ...
    def test_dockerfile_dev_python_base(self, dockerfile_dev):
        ...
        assert "python:3.11" in dockerfile_dev or "python:3.11" in dockerfile_dev.lower(), \
    ...
    def test_dockerfile_dev_system_deps(self, dockerfile_dev):
        ...
        deps = [
            ...
            "libgtk-3-0", "libavcodec-dev", "libavformat-dev"
        ]
        for dep in deps:
            assert dep in dockerfile_dev, \
                f"Должна быть установлена зависимость {dep}"

    def test_dockerfile_dev_dev_requirements(self, dockerfile_dev):
        """Тест: Dockerfile.dev устанавливает dev requirements."""
        assert "requirements-dev.txt" in dockerfile_dev, \
    ...
    def test_dockerfile_dev_hot_reload(self, dockerfile_dev):
        ...
        assert "--reload" in dockerfile_dev, \
            "uvicorn должен запускаться с --reload"
        assert "DEBUG=true" in dockerfile_dev or "ENVIRONMENT=development" in dockerfile_dev, \
    ...
    def test_dockerfile_dev_exposes_ports(self, dockerfile_dev):
        ...
        assert "EXPOSE 8000" in dockerfile_dev, \
            "Должен экспортировать порт 8000"

    def test_dockerfile_dev_creates_directories(self, dockerfile_dev):
        """Тест: Dockerfile.dev создает необходимые директории."""
        assert "mkdir -p" in dockerfile_dev, \
            "Должен создавать необходимые директории"
        assert "/app/logs" in dockerfile_dev or "logs" in dockerfile_dev, \
            "Должен создавать директорию для логов"
        assert "/app/uploads" in dockerfile_dev or "uploads" in dockerfile_dev, \
...
class TestDockerIgnoreFile:
    ...
    def test_dockerignore_excludes_git(self, dockerignore_content):
        ...
        assert ".git" in dockerignore_content, \
            ".dockerignore должен исключать .git"
    ...
    def test_dockerignore_excludes_env(self, dockerignore_content):
        ...
        assert ".env" in dockerignore_content, \
            ".dockerignore должен исключать .env"
    ...
    def test_dockerignore_excludes_tests(self, dockerignore_content):
        ...
        assert "tests" in dockerignore_content, \
            ".dockerignore должен исключать tests"
    ...
    def test_dockerignore_excludes_docker_files(self, dockerignore_content):
        ...
        has_dockerfile = "Dockerfile" in dockerignore_content or "*.dockerfile" in dockerignore_content
        assert has_dockerignore or has_dockerfile, \
    ...
    def test_dockerignore_excludes_readme(self, dockerignore_content):
        ...
        has_readme = "README" in dockerignore_content or "readme" in dockerignore_content.lower()
...
class TestDockerSecurity:
    ...
    def test_dockerfile_no_root_user(self, dockerfile_prod):
        ...
        assert "USER" in dockerfile_prod, \
            "Dockerfile должен переключаться на non-root пользователя"
        assert "USER root" not in dockerfile_prod, \
    ...
    def test_dockerfile_no_passwords_in_image(self, dockerfile_prod):
        ...
        assert "password" not in content_lower or "PASSWORD" not in dockerfile_prod, \
            "Dockerfile не должен содержать пароли в явном виде"
        assert "secret" not in content_lower or "SECRET" not in dockerfile_prod, \
    ...
    def test_docker_compose_prod_debug_false(self, docker_compose_prod):
        ...
        assert str(debug_value).lower() == "false", \
    ...
    def test_docker_compose_prod_log_level_info(self, docker_compose_prod):
        ...
        assert log_level.upper() == "INFO", \
            "LOG_LEVEL должен быть INFO в production"
