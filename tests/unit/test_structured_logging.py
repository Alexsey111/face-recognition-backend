...
class TestStructuredLogging:
    ...
    def test_redact_sensitive_data_nested(self):
        ...
        data = {
            "user": {
                "name": "John Doe",
                "credentials": {"password": "secret", "token": "jwt_token"},
            },
            "normal_data": "public",
        }
    ...
    def test_logger_factory(self):
        ...
        logger = LoggerFactory.setup_logger(
            name="test_logger", level="DEBUG", log_format="text"
        )
    ...
    def test_logger_factory_with_file(self):
        ...
        try:
            ...
            logger = LoggerFactory.setup_logger(
                name="file_logger", level="INFO", log_file=temp_path, log_format="text"
            )
    ...
    def test_log_entry_chaining(self):
        ...
        with patch("app.utils.structured_logging.get_logger") as mock_get_logger:
            ...
            create_log("Test message").with_request_id("req-123").with_user_id(
                "user-456"
            ).with_ip_address("192.168.1.1").with_tags(["test", "logging"]).with_extra(
    ...
    def test_audit_logger_convenience_methods(self):
        ...
        with patch("app.utils.structured_logging.get_logger") as mock_get_logger:
            ...
            audit_logger.auth_failed(
                user_id="user-456", error_message="Invalid password"
            )
    ...
    def test_redact_sensitive_data_depth_limit(self):
        ...
        assert (
            redacted["level1"]["level2"]["level3"]["level4"]["level5"]["password"]
            == "secret"
        )
    ...
    def test_log_rotation_concept(self):
        ...
        assert (
            should_rotate_log(100 * 1024 * 1024, 100 * 1024 * 1024, 10) is True
        )  # Равно max
        assert (
            should_rotate_log(150 * 1024 * 1024, 100 * 1024 * 1024, 10) is True
        )  # Больше max
        assert (
            should_rotate_log(50 * 1024 * 1024, 100 * 1024 * 1024, 10) is False
        )  # Меньше max
    ...
    def test_redact_sensitive_data_nested_structures(self):
        ...
        def redact_sensitive_data(data, depth=0, max_depth=5):
            ...
            if isinstance(data, dict):
                ...
                return [
                    redact_sensitive_data(item, depth + 1, max_depth) for item in data
                ]
            else:
                return data
