...
class DatabaseService:
    ...
    async def get_active_references_by_user(self, user_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"Error getting active references for user {user_id}: {str(e)}")
    ...
    async def update_reference(self, reference_id: str, update_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        Обновление эталона.
    ...
    async def create_verification_session(self, session_data: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    async def get_total_requests_count(self, date_from: datetime, date_to: datetime) -> int:
        """
    ...
    async def get_successful_requests_count(self, date_from: datetime, date_to: datetime) -> int:
        """
    ...
    async def get_failed_requests_count(self, date_from: datetime, date_to: datetime) -> int:
        """
    ...
    async def get_average_response_time(self, date_from: datetime, date_to: datetime) -> float:
        """
        ...
        try:
            async with self.db_session() as session:
                result = await session.execute(
                    select(func.avg(VerificationSession.processing_time))
                    .where(and_(
                        VerificationSession.created_at >= date_from,
                        VerificationSession.created_at <= date_to,
    ...
    async def get_verification_stats(self, date_from: datetime, date_to: datetime) -> Dict[str, int]:
        """
            ...
            async with self.db_session() as session:
                ...
                result = await session.execute(
                    ...
                    .where(and_(
                        VerificationSession.created_at >= date_from,
                        VerificationSession.created_at <= date_to,
                        VerificationSession.session_type.in_(["verification", "liveness"])
                    ))
    ...
    async def get_liveness_stats(self, date_from: datetime, date_to: datetime) -> Dict[str, int]:
        """
    ...
    async def get_user_stats(self, date_from: datetime, date_to: datetime) -> Dict[str, Any]:
        """
        ...
        try:
            async with self.db_session() as session:
                ...
                active_users = await session.scalar(
                    select(func.count(func.distinct(VerificationSession.user_id)))
                    .where(and_(
                        VerificationSession.created_at >= date_from,
                        VerificationSession.created_at <= date_to,
    ...
    async def get_references_statistics(self) -> Dict[str, Any]:
        ...
        try:
            async with self.db_session() as session:
                ...
                total_references = await session.scalar(select(func.count(Reference.id)))
                ...
                avg_quality = await session.scalar(
                    select(func.avg(Reference.quality_score))
                    .where(Reference.quality_score.is_not(None))
                )
                
                # Общее количество использований
                total_usage = await session.scalar(select(func.sum(Reference.usage_count)))
                ...
                return {
                    ...
                    "quality_distribution": {
                        "excellent": await self._get_references_by_quality_range(0.9, 1.0),
    ...
    async def get_sessions_statistics(self) -> Dict[str, Any]:
        ...
        try:
            async with self.db_session() as session:
                ...
                total_sessions = await session.scalar(select(func.count(VerificationSession.id)))
                
                # Сессии по статусам
                status_counts = {}
                for status in ["pending", "processing", "completed", "failed", "expired", "cancelled"]:
                    count = await session.scalar(
                        select(func.count(VerificationSession.id))
                        .where(VerificationSession.status == status)
                    )
                    status_counts[status] = count
                
                # Сессии по типам
                type_counts = {}
                for session_type in ["verification", "liveness", "enrollment", "identification"]:
                    count = await session.scalar(
                        select(func.count(VerificationSession.id))
                        .where(VerificationSession.session_type == session_type)
                    )
                ...
                avg_processing_time = await session.scalar(
                    select(func.avg(VerificationSession.processing_time))
                    .where(VerificationSession.processing_time.is_not(None))
                )
                ...
                return {
                    ...
                    "active_sessions": status_counts.get("pending", 0) + status_counts.get("processing", 0),
    ...
    def _session_to_dict(self, session_obj: VerificationSession) -> Dict[str, Any]:
        ...
        return {
            ...
            "reference_id": str(session_obj.reference_id) if session_obj.reference_id else None,
    ...
    async def _get_references_by_quality_range(self, min_quality: float, max_quality: float) -> int:
        """
        ...
        try:
            async with self.db_session() as session:
                result = await session.execute(
                    select(func.count(Reference.id)).where(and_(
