...
class EncryptionService:
    ...
    async def encrypt_embedding(self, embedding: bytes) -> bytes:
        ...
        try:
            ...
            logger.debug(f"Embedding encrypted successfully (original size: {len(embedding)} bytes)")
    ...
    async def decrypt_embedding(self, encrypted_embedding: bytes) -> bytes:
        ...
        try:
            ...
            if metadata.get("algorithm") != self.algorithm:
                raise EncryptionError(f"Algorithm mismatch: expected {self.algorithm}, got {metadata.get('algorithm')}")
            ...
            if stored_hash and calculated_hash != stored_hash:
                raise EncryptionError("Hash verification failed - data may be corrupted")
            
            logger.debug(f"Embedding decrypted successfully (size: {len(embedding_bytes)} bytes)")
    ...
    async def hash_password(self, password: str, salt: Optional[bytes] = None) -> Tuple[str, str]:
        """
    ...
    async def verify_password(self, password: str, hashed_password: str, salt: str) -> bool:
        """
    ...
    async def decrypt_file(self, encrypted_file_path: str, output_path: Optional[str] = None) -> str:
        ...
        try:
            ...
            raise EncryptionError(f"Failed to decrypt file {encrypted_file_path}: {str(e)}")
