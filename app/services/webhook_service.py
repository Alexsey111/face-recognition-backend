...
class WebhookService:
    ...
    async def send_batch_webhooks(
        self,
        webhooks_data: List[Dict[str, Any]],
        webhook_url: Optional[str] = None
    ) -> Dict[str, int]:
        """
        ...
        for webhook_data in webhooks_data:
            task = asyncio.create_task(
                self._send_webhook(
                    ...
                    webhook_url=webhook_url or webhook_data.get("webhook_url") or settings.WEBHOOK_URL,
    ...
    async def _send_webhook(
        ...
        for attempt in range(self.max_retries + 1):
            try:
                logger.debug(f"Sending webhook {event_type} (attempt {attempt + 1}) to {webhook_url}")
                ...
                headers = {
                    ...
                    "X-Timestamp": payload.get("timestamp", datetime.now(timezone.utc).isoformat())
                }
                ...
                if response.status_code >= 200 and response.status_code < 300:
                    logger.info(f"Webhook {event_type} sent successfully (status: {response.status_code})")
                    return True
                elif response.status_code >= 400 and response.status_code < 500:
                    # Клиентские ошибки - не повторяем
                    logger.warning(f"Webhook {event_type} failed with client error {response.status_code}: {response.text}")
                    return False
                else:
                    # Серверные ошибки - повторяем
                    logger.warning(f"Webhook {event_type} failed with server error {response.status_code}: {response.text}")
                ...
                logger.warning(f"Webhook {event_type} connection error (attempt {attempt + 1}): {str(e)}")
                last_error = f"Connection error: {str(e)}"
                
            except Exception as e:
                logger.warning(f"Webhook {event_type} unexpected error (attempt {attempt + 1}): {str(e)}")
                last_error = str(e)
        ...
        logger.error(f"Webhook {event_type} failed after {self.max_retries + 1} attempts: {last_error}")
        return False
    ...
    async def validate_webhook_url(self, webhook_url: str) -> Dict[str, Any]:
        ...
        try:
            ...
            response = await self.client.get(
                webhook_url,
                headers={"User-Agent": "FaceRecognitionService/1.0"}
            )
