...
class MLService:
    ...
    async def health_check(self) -> bool:
        ...
        try:
            response = await self.client.get(
                f"{self.ml_service_url}/health", headers=self._get_headers()
            )
    ...
    async def verify_face(
        ...
        try:
            ...
            logger.info(
                f"Face verification: {verified} (similarity: {similarity_score:.3f}, threshold: {threshold})"
            )
    ...
    async def compare_faces(
        ...
        try:
            ...
            logger.info(
                f"Face comparison: similarity {similarity_score:.3f}, distance {distance:.3f}"
            )
    ...
    async def check_liveness(
        ...
        try:
            ...
            if not result.get("success", False):
                raise MLServiceError(
                    f"Liveness check failed: {result.get('error', 'Unknown error')}"
                )

            logger.info(
                f"Liveness check completed: {result.get('liveness_detected', False)} (confidence: {result.get('confidence', 0.0):.3f})"
            )
    ...
    async def process_image(
        self, image_data: bytes, image_url: Optional[str] = None
    ) -> Dict[str, Any]:
        """
    ...
    async def detect_faces(self, image_data: bytes) -> Dict[str, Any]:
        ...
        try:
            ...
            if not result.get("success", False):
                raise MLServiceError(
                    f"Face detection failed: {result.get('error', 'Unknown error')}"
                )
    ...
    async def batch_process_images(
        ...
        try:
            ...
            payload = {
                "images": [
                    base64.b64encode(img).decode("utf-8") for img in images_data
                ],
                "return_analysis": True,
            }
    ...
    async def get_model_info(self) -> Dict[str, Any]:
        ...
        try:
            response = await self.client.get(
                f"{self.ml_service_url}/model/info", headers=self._get_headers()
            )
