...
class UploadRequest(BaseModel):
    ...
    reference_id: Optional[str] = Field(
        None,
        description="ID эталонного изображения для сравнения"
    )
    ...
    def validate_image_data(cls, v):
        ...
        if v is not None:
            return UploadRequest.validate_image_data(v)
        return v
...
class ReferenceUpdateRequest(BaseModel):
    ...
    label: Optional[str] = Field(
        None,
        description="Новая метка эталона"
    )
    metadata: Optional[Dict[str, Any]] = Field(
        None,
...
class AdminStatsRequest(BaseModel):
    ...
    include_user_stats: bool = Field(
        False,
        description="Включить статистику по пользователям"
    )
    ...
    def validate_dates(cls, v, info: ValidationInfo):
        ...
        if v:
            try:
                ...
                raise ValueError(f"Invalid date format for {info.field_name}. Expected YYYY-MM-DD")
        return v
