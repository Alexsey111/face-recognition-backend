...
class WebhookConfigBase(BaseModel):
    ...
    timeout: int = Field(default=10, ge=1, le=60, description="Таймаут запроса в секундах")
...
class WebhookLogBase(BaseModel):
    ...
    attempts: int = Field(default=0, ge=0, le=10, description="Количество попыток отправки")
    last_attempt_at: Optional[datetime] = Field(None, description="Время последней попытки")
    next_retry_at: Optional[datetime] = Field(None, description="Время следующей попытки")
    status: WebhookStatus = Field(default=WebhookStatus.PENDING, description="Статус отправки")
...
class WebhookVerificationData(BaseModel):
    ...
    liveness_score: Optional[float] = Field(None, ge=0.0, le=1.0, description="Оценка живости")
    processing_time_ms: int = Field(..., ge=0, description="Время обработки в миллисекундах")
...
class WebhookTestRequest(BaseModel):
    ...
    event_type: WebhookEventType = Field(
        default=WebhookEventType.WEBHOOK_TEST, 
        description="Тип тестового события"
    )
    custom_data: Optional[Dict[str, Any]] = Field(None, description="Кастомные данные для теста")
...
class WebhookStatistics(BaseModel):
    ...
    last_webhook_at: Optional[datetime] = Field(None, description="Время последнего webhook")


class WebhookRetryRequest(BaseModel):
    """Модель запроса на повторную отправку webhook"""
    webhook_log_id: str = Field(..., description="ID записи лога для повторной отправки")
    force: bool = Field(default=False, description="Принудительно отправить игнорируя лимиты")
...
class WebhookBulkAction(BaseModel):
    ...
    action: str = Field(..., description="Действие (activate, deactivate, delete, test)")
