...
class FaceDetectionResult(BaseModel):
    ...
    bounding_box: Optional[List[float]] = Field(None, description="Координаты [x, y, width, height]")
    ...
    face_area_percentage: Optional[float] = Field(None, description="Процент площади лица на изображении")
    ...
    def validate_bounding_box(cls, v):
        ...
        if v is not None:
            if len(v) != 4:
                raise ValueError("Bounding box must have 4 coordinates [x, y, width, height]")
...
class FaceLandmarks(BaseModel):
    ...
    confidence: float = Field(0.0, ge=0.0, le=1.0, description="Уверенность детекции landmarks")
    
    # Специфичные метрики для лица
    eye_distance: Optional[float] = Field(None, description="Расстояние между глазами")
    face_ratio: Optional[float] = Field(None, description="Соотношение ширины к высоте лица")
...
class FaceEmbedding(BaseModel):
    ...
    quality_score: float = Field(0.0, ge=0.0, le=1.0, description="Оценка качества эмбеддинга")
    ...
    source_image_hash: Optional[str] = Field(None, description="Хеш исходного изображения")
    ...
    def validate_embedding(cls, v, info):
        ...
        if len(v) != dimension:
            raise ValueError(f"Embedding dimension mismatch: expected {dimension}, got {len(v)}")
...
class FaceEmbeddingComparison(BaseModel):
    ...
    """
    similarity_score: float = Field(..., ge=-1.0, le=1.0, description="Косинусная схожесть")
    ...
    confidence: float = Field(0.0, ge=0.0, le=1.0, description="Уверенность в результате")
    
    # Дополнительные метрики
    normalized_distance: Optional[float] = Field(None, description="Нормализованное расстояние [0, 1]")
    confidence_level: Optional[Literal["very_low", "low", "medium", "high", "very_high"]] = Field(
        None, description="Уровень уверенности"
    )
...
class FaceQualityAssessment(BaseModel):
    ...
    """
    overall_quality: float = Field(..., ge=0.0, le=1.0, description="Общая оценка качества")
    ...
    face_angle_score: float = Field(0.0, ge=0.0, le=1.0, description="Угол наклона лица")
    occlusion_score: float = Field(0.0, ge=0.0, le=1.0, description="Окклюзия (закрытие лица)")
    
    # Проблемы
    issues: List[str] = Field(default_factory=list, description="Список обнаруженных проблем")
    recommendations: List[str] = Field(default_factory=list, description="Рекомендации по улучшению")
    
    # Пороговая проверка
    passes_quality_check: bool = Field(..., description="Проходит ли минимальные требования")
    minimum_quality_threshold: float = Field(0.5, description="Минимальный порог качества")
...
class LightingAnalysis(BaseModel):
    ...
    """
    overall_quality: float = Field(..., ge=0.0, le=1.0, description="Общее качество освещения")
    
    # Компоненты освещения
    exposure_score: float = Field(0.0, ge=0.0, le=1.0, description="Экспозиция")
    shadow_evenness: float = Field(0.0, ge=0.0, le=1.0, description="Равномерность теней")
    left_right_balance: float = Field(0.0, ge=0.0, le=1.0, description="Баланс освещения слева/справа")
...
class DepthAnalysis(BaseModel):
    ...
    flatness_score: float = Field(..., ge=0.0, le=1.0, description="Оценка плоскости (0=3D, 1=плоско)")
    
    # Результат проверки
    is_likely_real: bool = Field(..., description="Вероятно реальное лицо (не фото/экран)")
    ...
    # Аномалии
    anomalies: List[str] = Field(default_factory=list, description="Обнаруженные аномалии")
    
    # Метод анализа
    analysis_method: str = Field(
        default="gradient_based", 
        description="Метод анализа глубины"
    )
...
class LivenessDetectionResult(BaseModel):
    ...
    detection_method: Literal[
        ...
        "certified", "depth_based", "video_based"
    ...
    texture_analysis: Optional[Dict[str, float]] = Field(None, description="Анализ текстуры")
    ...
    is_certified: bool = Field(
        False, 
        description="Проверка с сертифицированной моделью (MiniFASNetV2)"
    )
    certification_level: Optional[str] = Field(
        None, 
        description="Уровень сертификации (ISO, NIST, etc.)"
    )
...
class ActiveLivenessChallenge(BaseModel):
    ...
    challenge_id: str = Field(
        ...
        description="Уникальный ID челленджа"
    )
    ...
    detected_action: Optional[Dict[str, Any]] = Field(None, description="Обнаруженное действие")
    confidence: float = Field(0.0, ge=0.0, le=1.0, description="Уверенность в результате")
...
class FaceAlignmentResult(BaseModel):
    ...
    alignment_method: str = Field(default="68_point_landmarks", description="Метод выравнивания")
    ...
    alignment_quality: float = Field(0.0, ge=0.0, le=1.0, description="Качество выравнивания")
...
class ComprehensiveFaceAnalysis(BaseModel):
    ...
    alignment: Optional[FaceAlignmentResult] = Field(None, description="Результат выравнивания")
    ...
    # Liveness
    liveness: Optional[LivenessDetectionResult] = Field(None, description="Проверка живости")
    
    # Общий результат
    overall_pass: bool = Field(..., description="Общий результат проверки")
    overall_confidence: float = Field(0.0, ge=0.0, le=1.0, description="Общая уверенность")
    
    # Проблемы и рекомендации (сводные)
    all_issues: List[str] = Field(default_factory=list, description="Все обнаруженные проблемы")
    all_recommendations: List[str] = Field(
        default_factory=list, 
        description="Все рекомендации"
    )
...
class BatchFaceProcessingRequest(BaseModel):
    ...
    batch_id: str = Field(
        default_factory=lambda: str(uuid.uuid4()),
        description="Уникальный ID пакета"
    )
    images: List[str] = Field(..., min_length=1, max_length=100, description="Список изображений (base64)")
    ...
    # Настройки
    batch_size: int = Field(10, ge=1, le=50, description="Размер подпакета для обработки")
...
class BatchFaceProcessingResult(BaseModel):
    ...
    # Результаты
    results: List[Dict[str, Any]] = Field(default_factory=list, description="Результаты по каждому изображению")
    
    # Производительность
    total_processing_time: float = Field(0.0, description="Общее время обработки")
    average_time_per_image: float = Field(0.0, description="Среднее время на изображение")
    
    # Статус
    status: Literal["completed", "partial", "failed"] = Field(..., description="Статус обработки")
    timestamp: datetime = Field(
        default_factory=lambda: datetime.now(timezone.utc),
...
class SpoofingIndicators(BaseModel):
    ...
    """
    overall_spoof_score: float = Field(..., ge=0.0, le=1.0, description="Общая оценка подделки")
    is_likely_spoof: bool = Field(..., description="Вероятно подделка")
    
    # Типы атак
    print_attack_probability: float = Field(0.0, ge=0.0, le=1.0, description="Вероятность атаки фото")
    screen_replay_probability: float = Field(0.0, ge=0.0, le=1.0, description="Вероятность атаки экраном")
    mask_attack_probability: float = Field(0.0, ge=0.0, le=1.0, description="Вероятность атаки маской")
    deepfake_probability: float = Field(0.0, ge=0.0, le=1.0, description="Вероятность deepfake")
    ...
    paper_texture_detected: bool = Field(False, description="Обнаружена текстура бумаги")
    unnatural_eye_reflection: bool = Field(False, description="Неестественное отражение в глазах")
    
    # Флаги
    flags: List[str] = Field(default_factory=list, description="Список обнаруженных флагов подделки")
    confidence: float = Field(0.0, ge=0.0, le=1.0, description="Уверенность в оценке")
