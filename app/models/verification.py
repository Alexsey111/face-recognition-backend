...
class VerificationResult(BaseModel):
    ...
    reference_label: Optional[str] = Field(
        None, description="Метка использованного эталона"
    )
    ...
    embedding_distance: Optional[float] = Field(
        None, description="Евклидово расстояние между эмбеддингами"
    )
    metadata: Optional[Dict[str, Any]] = Field(
        None, description="Дополнительные метаданные"
    )
...
class LivenessResult(BaseModel):
    ...
    depth_analysis: Optional[Dict[str, float]] = Field(
        None, description="Анализ глубины изображения"
    )
...
class SessionStats(BaseModel):
    ...
    liveness_success_rate: float = Field(
        ..., description="Процент успешных проверок живости"
    )
