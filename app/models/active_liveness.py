...
class ActiveLivenessChallengeRequest(BaseModel):
    ...
    challenge_type: ChallengeType = Field(
        default="random", description="Тип челленджа для проверки живости"
    )

    timeout_seconds: int = Field(
        default=10, ge=5, le=30, description="Время на выполнение челленджа (секунды)"
    )

    difficulty: Literal["easy", "medium", "hard"] = Field(
        default="medium", description="Сложность челленджа"
    )
...
class ActiveLivenessVerifyRequest(BaseModel):
    ...
    challenge_id: str = Field(..., description="ID челленджа, полученный при создании")

    video_data: Optional[bytes] = Field(
        default=None, description="Видео в формате MP4/WebM (base64 encoded)"
    )

    image_sequence: Optional[List[bytes]] = Field(
        default=None, description="Последовательность изображений (если нет видео)"
    )

    metadata: Optional[Dict[str, Any]] = Field(
        default=None, description="Дополнительные метаданные (FPS, device info, etc.)"
    )
    ...
    def validate_data_provided(cls, v, info):
        ...
        if (
            info.data.get("video_data") is None
            and info.data.get("image_sequence") is None
        ):
    ...
    class Config:
        json_schema_extra = {
            "example": {
                ...
                "metadata": {"fps": 30, "device": "iPhone 13", "camera": "front"},
            }
        }
...
class BlinkDetectionRequest(BaseModel):
    ...
    min_blinks: int = Field(
        default=1, ge=1, le=5, description="Минимальное количество морганий"
    )
    timeout_seconds: int = Field(default=10, ge=5, le=30)

    class Config:
        json_schema_extra = {"example": {"min_blinks": 2, "timeout_seconds": 10}}
...
class HeadMovementRequest(BaseModel):
    ...
    class Config:
        json_schema_extra = {
            "example": {"movement_type": "left", "min_angle_degrees": 20.0}
        }
...
class ChallengeInstruction(BaseModel):
    ...
    class Config:
        json_schema_extra = {
            "example": {"text": "Моргните 2 раза", "icon": "👁️", "duration_seconds": 10}
        }
...
class ActiveLivenessChallengeResponse(BaseModel):
    ...
    expected_duration_seconds: int = Field(
        ..., description="Ожидаемая длительность действия"
    )
    min_frames_required: int = Field(
        default=30, description="Минимум кадров для анализа"
    )
...
class BlinkDetectionResult(BaseModel):
    ...
    blink_frames: List[int] = Field(
        default_factory=list, description="Номера кадров с морганиями"
    )
    total_frames_analyzed: int = Field(..., description="Всего проанализировано кадров")

    quality_issues: List[str] = Field(
        default_factory=list, description="Проблемы с качеством"
    )
...
class HeadMovementResult(BaseModel):
    ...
    # Euler angles
    yaw: float = Field(
        ..., description="Поворот вокруг вертикальной оси (влево/вправо)"
    )
...
class OcclusionDetectionResult(BaseModel):
    ...
    occlusion_score: float = Field(
        ..., ge=0.0, le=1.0, description="Оценка видимости лица (1=полностью видимо)"
    )
    confidence: float = Field(..., ge=0.0, le=1.0)
...
class ActiveLivenessVerifyResponse(BaseModel):
    ...
    passive_liveness_score: float = Field(
        ..., ge=0.0, le=1.0, description="Оценка пассивной живости"
    )
    anti_spoofing_score: float = Field(
        ..., ge=0.0, le=1.0, description="Anti-spoofing оценка"
    )
    ...
    failure_reasons: List[str] = Field(
        default_factory=list, description="Причины неудачи"
    )
...
class SmileDetectionResult(BaseModel):
    ...
    smile_intensity: float = Field(
        ..., ge=0.0, le=1.0, description="Интенсивность улыбки"
    )
...
class CombinedLivenessScore(BaseModel):
    ...
    overall_score: float = Field(
        ..., ge=0.0, le=1.0, description="Общая оценка живости"
    )
    ...
    weights: Dict[str, float] = Field(
        default={"active": 0.5, "passive": 0.3, "anti_spoofing": 0.2}
    )
    ...
    class Config:
        json_schema_extra = {
            "example": {
                ...
                "weights": {"active": 0.5, "passive": 0.3, "anti_spoofing": 0.2},
