"""
API роуты для верификации лиц.
Endpoint для проверки соответствия лица эталонным изображениям.
"""
...
from ..models.verification import VerificationSessionCreate, VerificationRequest, VerificationResult
...
async def verify_face(request: VerifyRequest, http_request: Request):
    ...
    try:
        ...
        if request.reference_id:
            logger.info(f"Getting reference image {request.reference_id} for verification request {request_id}")
            ...
            if not reference_info.is_active:
                raise ValidationError(f"Reference image {request.reference_id} is not active")
            
            # Дешифрация эмбеддинга эталона
            if reference_info.embedding:
                encryption_service = EncryptionService()
                reference_embedding = await encryption_service.decrypt_embedding(reference_info.embedding)
        elif request.user_id:
            # Получение активных эталонов пользователя
            logger.info(f"Getting active references for user {request.user_id} in verification request {request_id}")
            user_references = await db_service.get_active_references_by_user(request.user_id)
            
            if not user_references:
                raise NotFoundError(f"No active references found for user {request.user_id}")
        ...
        logger.info(f"Processing image with ML service for verification request {request_id} (threshold={threshold_used})")
        ...
        if not ml_result.get("success", False):
            raise ProcessingError(f"ML verification failed: {ml_result.get('error', 'Unknown error')}")
        ...
        if verified and request.auto_enroll and request.user_id:
            logger.info(f"Auto-enrolling verified face for user {request.user_id} in verification request {request_id}")
            try:
        ...
        logger.info(f"Face verification completed: {verified} (confidence: {confidence:.3f}), request {request_id}")
        return response
        
    except (ValidationError, NotFoundError) as e:
        logger.warning(f"Validation/NotFound error for verification request {request_id}: {str(e)}")
        raise HTTPException(
            status_code=400,
            detail={
                "success": False,
        ...
        logger.error(f"Processing error for verification request {request_id}: {str(e)}")
        raise HTTPException(
            status_code=422,
            detail={
                "success": False,
        ...
        logger.error(f"Unexpected error for verification request {request_id}: {str(e)}")
        raise HTTPException(
            status_code=500,
            detail={
                "success": False,
...
async def create_verification_session(request: VerificationSessionCreate, http_request: Request):
    """
    ...
    try:
        ...
        logger.error(f"Error creating verification session, request {request_id}: {str(e)}")
        raise HTTPException(
            status_code=500,
            detail={
                "success": False,
...
async def verify_face_in_session(session_id: str, request: VerificationRequest, http_request: Request):
    """
    ...
    try:
        logger.info(f"Starting verification in session {session_id}, request {request_id}")
        ...
        if not session_data:
            raise NotFoundError(f"Verification session {session_id} not found or expired")
        ...
        await cache_service.set_verification_session(session_id, session_data, expire_seconds=1800)  # 30 минут
        ...
        await cache_service.set_verification_session(session_id, session_data, expire_seconds=3600)  # 1 час
        
        logger.info(f"Verification in session {session_id} completed successfully, request {request_id}")
        return verify_response
        
    except (ValidationError, NotFoundError) as e:
        logger.warning(f"Validation/NotFound error in verification session {session_id}, request {request_id}: {str(e)}")
        ...
        if session_id:
            try:
                ...
                if session_data:
                    ...
                    await cache_service.set_verification_session(session_id, session_data, expire_seconds=3600)
            except Exception:
                pass
        ...
        logger.error(f"Unexpected error in verification session {session_id}, request {request_id}: {str(e)}")
        raise HTTPException(
            status_code=500,
            detail={
                "success": False,
...
async def auto_enroll_reference(user_id: str, image_data: str, verification_result: dict, metadata: dict):
    """
