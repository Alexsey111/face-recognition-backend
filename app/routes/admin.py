"""
API роуты для администраторских функций.
Endpoints для управления системой, статистики и мониторинга.
"""
...
from ..utils.exceptions import ValidationError, ProcessingError, NotFoundError, UnauthorizedError
...
async def get_admin_stats(
    ...
    include_user_stats: bool = Query(False, description="Включить статистику по пользователям"),
    include_performance: bool = Query(True, description="Включить метрики производительности"),
    ...
    try:
        ...
        successful_requests = await db_service.get_successful_requests_count(date_from, date_to)
        failed_requests = await db_service.get_failed_requests_count(date_from, date_to)
        average_response_time = await db_service.get_average_response_time(date_from, date_to)
        ...
        logger.warning(f"Validation error getting admin stats, request {request_id}: {str(e)}")
        raise HTTPException(
            status_code=400,
            detail={
                "success": False,
...
async def get_all_users(
    ...
    try:
        ...
        allowed_sort_fields = ["created_at", "updated_at", "username", "email", "last_login"]
        ...
        logger.warning(f"Validation error getting users, request {request_id}: {str(e)}")
        raise HTTPException(
            status_code=400,
            detail={
                "success": False,
...
async def toggle_maintenance_mode(
    ...
    try:
        ...
        logger.info(f"Maintenance mode {'enabled' if enabled else 'disabled'} by admin, request {request_id}")
