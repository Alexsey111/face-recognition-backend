...
def to_webhook_log_response(
    log: WebhookLog, config: WebhookConfig
) -> WebhookLogResponse:
...
async def create_webhook_config(
    ...
    try:
        ...
        is_secure, security_error = validate_webhook_url_security(
        ...
        validation_result = await webhook_service.validate_webhook_url(
            config_data.webhook_url
        )
        ...
        logger.info(
            f"Webhook config created for user {current_user.id}: {webhook_config.id}"
        )
    ...
    try:
        ...
        if "event_types" in update_data:
            update_data["event_types"] = [
                event_type.value for event_type in update_data["event_types"]
            ]
...
async def test_webhook(
    ...
    try:
            ...
            result = await db.execute(
                ...
                .where(
                    and_(
                        ...
                        WebhookConfig.event_types.contains(
        ...
        success = await webhook_service._send_webhook(
            payload=payload, webhook_url=webhook_url, event_type="test"
        )
...
async def get_webhook_logs(
    ...
    event_type: Optional[WebhookEventType] = Query(
...
async def get_webhook_statistics(
    ...
    try:
        ...
        successful_webhooks = len(
            [log for log, config in logs if log.status == WebhookStatus.SUCCESS]
        )
        failed_webhooks = len(
            [log for log, config in logs if log.status == WebhookStatus.FAILED]
        )
        pending_webhooks = len(
            [log for log, config in logs if log.status == WebhookStatus.PENDING]
        )
        retry_webhooks = len(
            [log for log, config in logs if log.status == WebhookStatus.RETRY]
        )
        ...
        response_times = [
            log.processing_time for log, config in logs if log.processing_time
        ]
...
async def retry_webhook(
    ...
    try:
        ...
        if (
            log.status not in [WebhookStatus.FAILED, WebhookStatus.EXPIRED]
            and not retry_request.force
        ):
            raise HTTPException(
                status_code=400, detail=f"Cannot retry webhook with status {log.status}"
            )
...
async def bulk_webhook_action(
    ...
    try:
        ...
        if len(configs) != len(action.webhook_config_ids):
            raise HTTPException(
                status_code=404, detail="Some webhook configs not found"
            )
        ...
        if action.action == "activate":
            ...
            for config in configs:
                ...
                asyncio.create_task(
                    webhook_service._send_webhook_with_config(
                        payload, config, str(uuid.uuid4())
                    )
                )
                updated_count += 1
        else:
        ...
        logger.info(
            f"Bulk action '{action.action}' performed on {updated_count} webhook configs"
        )
