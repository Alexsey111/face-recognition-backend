    ...
    try:
        ...
        if not file.content_type.startswith("image/"):
            raise ValidationError(
                f"Invalid file type: {file.content_type}. Expected image."
            )
...
async def check_video_liveness(
    ...
    try:
        ...
        if not file.content_type.startswith("video/"):
            raise ValidationError(
                f"Invalid file type: {file.content_type}. Expected video."
            )
    ...
    **Response:**
    ...
    try:
        ...
        if movement_type not in valid_movements:
            raise ValidationError(
                f"Invalid movement_type. Must be one of: {valid_movements}"
            )
