"""
API роуты для проверки живости.
Endpoint для определения живости лица и антиспуфинга.
"""
...
from ..models.verification import VerificationSessionCreate, LivenessRequest as SessionLivenessRequest, LivenessResult
...
async def check_liveness(request: LivenessRequest, http_request: Request):
    ...
    try:
        ...
        if challenge_type in ("active", "blink", "smile", "turn_head") and not request.challenge_data:
            raise ValidationError(f"challenge_data is required for challenge_type={challenge_type}")
        ...
        logger.info(f"Processing image with ML service for liveness request {request_id}")
        ...
        if not ml_result.get("success", False):
            raise ProcessingError(f"ML liveness check failed: {ml_result.get('error', 'Unknown error')}")
        ...
        spoof_analysis = await validation_service.analyze_spoof_signs(validation_result.image_data)
        ...
        if spoof_analysis.get("flags"):
            recommendations = recommendations + [f"heuristic:{flag}" for flag in spoof_analysis["flags"]]
        ...
        logger.info(f"Liveness check completed: {liveness_detected} (confidence: {confidence:.3f}), request {request_id}")
        return response
...
async def create_liveness_session(request: VerificationSessionCreate, http_request: Request):
    """
    ...
    try:
        ...
        if request.session_type != "liveness":
            raise ValidationError("Session type must be 'liveness' for liveness session creation")
        ...
        logger.warning(f"Validation error creating liveness session, request {request_id}: {str(e)}")
        raise HTTPException(
            status_code=400,
            detail={
                "success": False,
...
async def check_liveness_in_session(session_id: str, request: SessionLivenessRequest, http_request: Request):
    """
    ...
    try:
        logger.info(f"Starting liveness check in session {session_id}, request {request_id}")
        ...
        logger.info(f"Liveness check in session {session_id} completed successfully, request {request_id}")
        return liveness_response
        
    except (ValidationError, NotFoundError) as e:
        logger.warning(f"Validation/NotFound error in liveness session {session_id}, request {request_id}: {str(e)}")
        ...
        logger.error(f"Unexpected error in liveness session {session_id}, request {request_id}: {str(e)}")
        raise HTTPException(
            status_code=500,
            detail={
                "success": False,
...
async def get_liveness_session_status(session_id: str, http_request: Request):
    ...
    try:
        logger.info(f"Getting liveness session status for {session_id}, request {request_id}")
        ...
        if session_data.get("status") == "completed" and session_data.get("response_data"):
        ...
        logger.info(f"Liveness session status retrieved for {session_id}, request {request_id}")
        return response
        
    except (ValidationError, NotFoundError) as e:
        logger.warning(f"Validation/NotFound error getting liveness session status {session_id}, request {request_id}: {str(e)}")
        raise HTTPException(
            status_code=400,
            detail={
                "success": False,
        ...
        logger.error(f"Unexpected error getting liveness session status {session_id}, request {request_id}: {str(e)}")
        raise HTTPException(
            status_code=500,
            detail={
                "success": False,
