"""
API роуты для работы с эталонными изображениями.
Endpoints для управления эталонами пользователей.
"""
...
async def get_references(
    ...
    quality_min: float = Query(None, ge=0.0, le=1.0, description="Минимальное качество"),
    quality_max: float = Query(None, ge=0.0, le=1.0, description="Максимальное качество"),
    ...
    try:
        ...
        allowed_sort_fields = ["created_at", "updated_at", "quality_score", "usage_count", "label"]
        if sort_by not in allowed_sort_fields:
            raise ValidationError(f"Invalid sort_by field. Allowed: {allowed_sort_fields}")
        ...
        logger.warning(f"Validation error getting references, request {request_id}: {str(e)}")
        raise HTTPException(
            status_code=400,
            detail={
                "success": False,
...
async def get_reference(reference_id: str, http_request: Request):
    ...
    try:
        ...
        logger.info(f"Reference {reference_id} retrieved successfully, request {request_id}")
        return response
        
    except NotFoundError as e:
        logger.warning(f"Reference {reference_id} not found, request {request_id}: {str(e)}")
        raise HTTPException(
            status_code=404,
            detail={
                "success": False,
        ...
        logger.error(f"Error getting reference {reference_id}, request {request_id}: {str(e)}")
        raise HTTPException(
            status_code=500,
            detail={
                "success": False,
...
async def create_reference(request: ReferenceCreateRequest, http_request: Request):
    ...
    try:
        logger.info(f"Creating reference for user {request.user_id}, request {request_id}")
        ...
        embedding_result = await ml_service.generate_embedding(validation_result.image_data)
        
        if not embedding_result.get("success"):
            raise ProcessingError("Failed to generate embedding for reference")
        
        # Шифрование эмбеддинга
        encrypted_embedding = await encryption_service.encrypt_embedding(embedding_result["embedding"])
        
        # Проверка качества
        quality_score = embedding_result.get("quality_score", 0.0)
        if quality_score < request.quality_threshold:
            raise ValidationError(f"Image quality {quality_score} is below threshold {request.quality_threshold}")
        ...
        reference_data = {
            ...
            "file_size": upload_result.get("file_size") if upload_result else len(validation_result.image_data),
        ...
        if settings.STORE_ORIGINAL_IMAGES and settings.DELETE_SOURCE_AFTER_PROCESSING and upload_result:
            try:
                await storage_service.delete_image(upload_result.get("image_id"))
            except Exception as cleanup_error:
                logger.warning(f"Failed to delete reference source image {upload_result.get('image_id')}: {cleanup_error}")
        ...
        logger.info(f"Reference created successfully: {created_reference['id']}, request {request_id}")
        return response
        
    except ValidationError as e:
        logger.warning(f"Validation error creating reference, request {request_id}: {str(e)}")
        raise HTTPException(
            status_code=400,
            detail={
                "success": False,
        ...
        logger.error(f"Processing error creating reference, request {request_id}: {str(e)}")
        raise HTTPException(
            status_code=422,
            detail={
                "success": False,
        ...
        logger.error(f"Unexpected error creating reference, request {request_id}: {str(e)}")
        raise HTTPException(
            status_code=500,
            detail={
                "success": False,
...
async def update_reference(reference_id: str, request: ReferenceUpdateRequest, http_request: Request):
    """
    ...
    try:
        ...
        if request.image_data:
            ...
            if not validation_result.is_valid:
                raise ValidationError(f"New image validation failed: {validation_result.error_message}")
            ...
            if not embedding_result.get("success"):
                raise ProcessingError("Failed to generate embedding for updated reference")
            ...
            quality_score = embedding_result.get("quality_score", 0.0)
            quality_threshold = request.quality_threshold or existing_reference.get("quality_threshold", 0.8)
            
            if quality_score < quality_threshold:
                raise ValidationError(f"New image quality {quality_score} is below threshold {quality_threshold}")
            
            # Обновление данных изображения
            update_data.update({
                "file_url": upload_result.get("file_url") if upload_result else existing_reference.get("file_url"),
                "file_size": upload_result.get("file_size") if upload_result else len(validation_result.image_data),
                "image_format": validation_result.image_format,
                "image_dimensions": validation_result.dimensions,
                "embedding": encrypted_embedding,
            ...
            if settings.STORE_ORIGINAL_IMAGES and settings.DELETE_SOURCE_AFTER_PROCESSING and upload_result:
                try:
                    ...
                    logger.warning(f"Failed to delete updated reference source image {upload_result.get('image_id')}: {cleanup_error}")
        ...
        logger.info(f"Reference {reference_id} updated successfully, request {request_id}")
        return response
        
    except (ValidationError, NotFoundError) as e:
        logger.warning(f"Validation/NotFound error updating reference {reference_id}, request {request_id}: {str(e)}")
        raise HTTPException(
            status_code=400,
            detail={
                "success": False,
        ...
        logger.error(f"Processing error updating reference {reference_id}, request {request_id}: {str(e)}")
        raise HTTPException(
            status_code=422,
            detail={
                "success": False,
        ...
        logger.error(f"Unexpected error updating reference {reference_id}, request {request_id}: {str(e)}")
        raise HTTPException(
            status_code=500,
            detail={
                "success": False,
...
async def update_reference_alias(reference_id: str, request: ReferenceUpdateRequest, http_request: Request):
    """
...
async def delete_reference(reference_id: str, http_request: Request):
    ...
    try:
        ...
        logger.info(f"Reference {reference_id} deleted successfully, request {request_id}")
        return response
        ...
        logger.error(f"Error deleting reference {reference_id}, request {request_id}: {str(e)}")
        raise HTTPException(
            status_code=500,
            detail={
                "success": False,
...
async def compare_with_references(request: ReferenceCompare, http_request: Request):
    ...
    try:
        ...
        for reference in references[:request.max_results]:
            try:
                ...
                if reference.get("embedding"):
                    reference_embedding = await encryption_service.decrypt_embedding(reference["embedding"])
                    ...
                    if compare_result.get("success"):
                        results.append({
                            ...
                            "similarity_score": compare_result.get("similarity_score", 0.0),
                            "quality_score": reference.get("quality_score"),
                            "distance": compare_result.get("distance"),
                            "processing_time": compare_result.get("processing_time", 0.0),
                            "metadata": reference.get("metadata") if request.include_metadata else None
                        })
            except Exception as e:
                logger.warning(f"Error comparing with reference {reference['id']}: {str(e)}")
                continue
        ...
        logger.info(f"Reference comparison completed: {len(results)} matches found, request {request_id}")
        return response
        
    except ValidationError as e:
        logger.warning(f"Validation error in reference comparison, request {request_id}: {str(e)}")
        raise HTTPException(
            status_code=400,
            detail={
                "success": False,
        ...
        logger.warning(f"NotFound error in reference comparison, request {request_id}: {str(e)}")
        raise HTTPException(
            status_code=404,
            detail={
                "success": False,
        ...
        logger.error(f"Unexpected error in reference comparison, request {request_id}: {str(e)}")
        raise HTTPException(
            status_code=500,
            detail={
                "success": False,
