...
class FaceAligner:
    ...
    async def align_face(
        ...
        try:
            ...
            face_data = await asyncio.to_thread(
                self._detect_face_with_landmarks, image
            )
    ...
    def _detect_face_with_landmarks(self, image: Image.Image) -> Optional[Dict[str, Any]]:
    ...
    def _apply_alignment(self, image: Image.Image, landmarks: np.ndarray) -> Image.Image:
        """
    ...
    def _calculate_rotation_angle(self, left_eye: Tuple[float, float], right_eye: Tuple[float, float]) -> float:
    ...
    def _get_landmarks_coords(self, landmarks: np.ndarray) -> Dict[str, Tuple[float, float]]:
        """Получение именованных координат landmarks."""
        names = ["left_eye", "right_eye", "nose", "left_mouth", "right_mouth"]
        return {name: (float(landmarks[i][0]), float(landmarks[i][1])) 
                for i, name in enumerate(names) if i < len(landmarks)}
