...
def log_error(
    ...
    **kwargs,
    ...
    log_with_context(
        logger, logging.ERROR, message, request_id, user_id, extra=extra, **kwargs
    )
...
def log_critical(
    ...
    **kwargs,
    ...
    if error:
        extra["error"] = str(error)
        extra["error_type"] = (
            error.__class__.__name__ if hasattr(error, "__class__") else "UnknownError"
        )

    log_with_context(
        logger, logging.CRITICAL, message, request_id, user_id, extra=extra, **kwargs
    )
