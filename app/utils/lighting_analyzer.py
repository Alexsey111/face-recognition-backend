...
class LightingAnalyzer:
    ...
    async def analyze(
        ...
        try:
            ...
            shadow_analysis = await asyncio.to_thread(
                self._analyze_shadows, face_img
            )
    ...
    def _analyze_brightness(self, face_img: np.ndarray) -> Dict[str, Any]:
        ...
        if mean_brightness < self.BRIGHTNESS_MIN:
            ...
            exposure_score = 1.0 - (mean_brightness - self.BRIGHTNESS_MAX) / (255 - self.BRIGHTNESS_MAX)
    ...
    def _analyze_shadows(self, face_img: np.ndarray) -> Dict[str, Any]:
        ...
        if shadow_areas:
            ...
            shadow_distribution = len(shadow_areas) / (total_pixels / 10000)  # плотность теней
        else:
            avg_shadow_size = 0
            max_shadow_size = 0
    ...
    def _analyze_light_symmetry(self, face_img: np.ndarray) -> Dict[str, Any]:
        ...
        brightness_diff = np.abs(left_half_cropped.astype(float) - right_half_cropped.astype(float))
        ...
        brightness_ratio = min(left_mean, right_mean) / (max(left_mean, right_mean) + 1e-6)
        ...
        vertical_brightness_ratio = min(top_mean, bottom_mean) / (max(top_mean, bottom_mean) + 1e-6)
