...
def validate_image_size(image_data: Union[str, bytes], max_size: int = FILE_LIMITS["max_image_size"]) -> bool:
    """
...
def validate_file_hash(data: Union[str, bytes], expected_hash: str, algorithm: str = "sha256") -> bool:
    """
...
def sanitize_string(text: str, max_length: int = None, allowed_chars: str = None) -> str:
    ...
    if allowed_chars:
        ...
        sanitized = ''.join(c for c in text if c in allowed_chars or c.isalnum() or c.isspace())
    else:
