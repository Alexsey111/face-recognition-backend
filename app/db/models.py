...
class User(Base):
    ...
    references = relationship(
        "Reference", back_populates="user", cascade="all, delete-orphan"
    )
    ...
    system_configs = relationship(
        "SystemConfig", back_populates="updated_by_user", cascade="all, delete-orphan"
    )
...
class Reference(Base):
    ...
    previous_reference_id = Column(
        String(36), ForeignKey("references.id", ondelete="SET NULL"), nullable=True
    )
    ...
    verification_sessions = relationship(
        "VerificationSession", back_populates="reference", cascade="all, delete-orphan"
    )
...
class WebhookConfig(Base):
    ...
    webhook_logs = relationship(
        "WebhookLog", back_populates="webhook_config", cascade="all, delete-orphan"
    )

    def __repr__(self):
        return (
            f"<WebhookConfig(user_id={self.user_id}, url={self.webhook_url[:50]}...)>"
        )
...
class WebhookLog(Base):
    ...
    webhook_config_id = Column(
        String(36), ForeignKey("webhook_configs.id", ondelete="CASCADE"), nullable=False
    )
