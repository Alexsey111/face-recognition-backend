...
class BaseCRUD:
    ...
    async def update(
        self, db: AsyncSession, id: str, obj_data: Dict[str, Any]
    ) -> Optional[Any]:
        """
        Обновление записи.
...
class UserCRUD(BaseCRUD):
    ...
    async def increment_stats(
        self, db: AsyncSession, user_id: str, stats_type: str, increment: int = 1
    ) -> bool:
        ...
        try:
            ...
            await db.rollback()
            logger.error(
                f"Error incrementing {stats_type} for user {user_id}: {str(e)}"
            )
            return False
...
class ReferenceCRUD(BaseCRUD):
    ...
    async def get_by_label(
        self, db: AsyncSession, user_id: str, label: str
    ) -> Optional[Reference]:
        """Получение эталона по метке."""
        try:
            result = await db.execute(
                select(Reference).where(
                    and_(Reference.user_id == user_id, Reference.label == label)
                )
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(
                f"Error getting reference by label {label} for user {user_id}: {str(e)}"
            )
    ...
    async def increment_usage(self, db: AsyncSession, reference_id: str) -> bool:
        ...
        try:
            ...
            await db.rollback()
            logger.error(
                f"Error incrementing usage for reference {reference_id}: {str(e)}"
            )
            return False
...
class VerificationSessionCRUD(BaseCRUD):
    ...
    async def get_by_user(
        self, db: AsyncSession, user_id: str, session_type: Optional[str] = None
...
class ApiKeyCRUD(BaseCRUD):
    ...
    async def increment_requests(
        self, db: AsyncSession, api_key_id: str, successful: bool = True
    ) -> bool:
        ...
        try:
            ...
            await db.rollback()
            logger.error(
                f"Error incrementing requests for API key {api_key_id}: {str(e)}"
            )
            return False
